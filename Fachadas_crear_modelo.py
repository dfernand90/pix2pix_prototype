...
if __name__ == "__main__":
   ...
   # .\dataset\facades\train\
    ...
    IMG_WIDTH = 256
    IMG_HEIGHT = 256
    # averiguar que es: 3 por 3 colores
    OUTPUT_CHANNELS = 3

    # step 5: cargar dataset de entrenamiento
    train_dataset = tf.data.Dataset.list_files(f"{path_train}\\*.jpg")
    train_dataset = train_dataset.map(load_image_train,
                                    num_parallel_calls=tf.data.AUTOTUNE)
    train_dataset = train_dataset.shuffle(BUFFER_SIZE)
    train_dataset = train_dataset.batch(BATCH_SIZE)
    # step 6: cargar el test de validacion y/o prueba
    try:
    ...
    # step 8: alocar el generador
    generator = Generator()

    # step 9: parametros del modelo
    LAMBDA = 100
    loss_object = tf.keras.losses.BinaryCrossentropy(from_logits=True)
    discriminator = Discriminator()
    generator_optimizer = tf.keras.optimizers.Adam(2e-4, beta_1=0.5)
    discriminator_optimizer = tf.keras.optimizers.Adam(2e-4, beta_1=0.5)
    # step 10: alocar el checkpoint. al parecer no lo crea solo lo aloquea
    ...
    # step 11: logs y mensajes para el usuario 
    log_dir="logs/"

    summary_writer = tf.summary.create_file_writer(
    log_dir + "fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))

    # step 12: entrenamiento
    fit(train_dataset, test_dataset, steps=500)
    # Save the generator model
    generator_save_path = './content/generator_model.keras'
